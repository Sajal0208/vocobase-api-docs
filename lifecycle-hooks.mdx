...
| `required: true` | A failure fails the call. |
| `timeoutMs` | How long to wait. Defaults to 3000. |

Keep `required` off unless the call is genuinely useless without the data — a slow CRM should not
drop customer calls.
