...
The tier of a call is determined by the **voice configuration** assigned to the agent. Agents without a voice configuration assigned default to **V1**.
...
### Cost examples (V1 default, ₹3.80/min)
...
The `credits_used` value of `9.50` above means **₹9.50** — the call was 127s actual → 135s billed (9 buckets) at the V1 default ₹3.80/min = `135 × 3.80 / 60 = ₹8.55` after rounding (worked example; check your transaction for exact rate at session time, since overrides may apply).
...
`amount` is in ₹. A USAGE row with `amount: -8.55` means **₹8.55 was deducted**. The transaction `metadata` field (returned via the dashboard's detailed view) includes the resolved `tier`, `rate_per_minute`, `rate_source` (`AGENT` / `PROJECT` / `ACCOUNT` / `DEFAULT`), `actual_seconds`, and `billed_seconds` for full audit.
...
If you use campaigns to batch outbound calls, the campaign status endpoint returns aggregate ₹ usage across all calls in the campaign.
