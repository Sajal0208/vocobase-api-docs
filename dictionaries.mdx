...
Returns every dictionary owned by the account, ordered by `created_at` descending. The list endpoint does not include `terms` -- fetch a single dictionary to see them.
