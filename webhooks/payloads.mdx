...
|---|---|---|
| `COMPLETED` | our bot | The bot ended the call — it said goodbye, the customer went quiet through the idle prompts, or the duration cap was reached. On the terminal `call.status.updated` event only, also used when the carrier says the call connected and ended normally but our bot has not reported yet. |
...
| `SCREENER` | our bot | A call screener answered. Takes precedence over carrier machine detection, so a screener never reads as `VOICEMAIL`. |
| `TRANSFERRED` | Vocobase | A transfer to a human was **initiated**. See the caveats below. |
...
| `UNKNOWN` | status | The call is marked complete, but nothing witnessed a conversation: our bot never reported on it (for example, the bot process died mid-call and the session was reconciled from the carrier's callback alone). The carrier cause is not consulted here. On `session.completed` and the `GET /calls` routes, a connected call with no bot report is `UNKNOWN`, never `COMPLETED`. |
...
- **`GET /calls` and `GET /calls/{call_id}` can disagree on a call whose session
  failed but whose carrier leg completed.** The single-call route reports the
  session's status (`failed`, and so typically `SYSTEM_ERROR`); the list reports the
  carrier's (`completed`, and so whatever the bot reported, or `UNKNOWN`). This is a pre-existing difference
  in how the two routes report `status`, which `end_call_reason` now follows.
  `GET /calls/{call_id}` is the more complete of the two.
- **`GET /calls/{call_id}` can report a transient `failed` / `SYSTEM_ERROR`** in
  the short window after the carrier has ended a call but before our bot's
  end-of-call report has been persisted (typically seconds; longer if the report
  is being retried). Once the report lands, the same call reads the bot's reason.
  If you poll immediately after `call.status.updated`, prefer to wait for
  `session.completed` instead.
