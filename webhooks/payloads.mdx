...
| `call.status` | string | Call lifecycle/carrier status: `completed`, `failed`, `no_answer`, `busy`, `canceled`. Voicemail is exposed through `call.disposition`, not `call.status`. |
