...
| `processing_complete` | boolean | `true` when the post-call pipeline (status flip, transcript persisted, recording uploaded, extraction run) has finished. Always `true` on the webhook payload itself — the webhook fires after the pipeline completes. Use it when polling `GET /calls/{id}` to tell "final" from "still decorating". |
| `pipeline_completed_at` | string (ISO 8601) or null | Timestamp when the pipeline finished. Non-null on the webhook. `null` when polling `GET /calls/{id}` and the pipeline hasn't completed yet, OR on rows created before this field shipped (treat null + `status: completed` for pre-existing rows as "final"). |
...
When an agent transfers a live call to a human (cold transfer via Plivo or Vobiz), **the bot leaves the room** the moment the carrier accepts the redirect. Concretely:

1. The bot speaks the configured announcement (`transfer_message`).
2. Backend POSTs to the carrier's call-redirect API.
3. The bot pushes `EndTaskFrame` upstream and the WebSocket closes.
4. The carrier bridges the caller leg directly to `transfer_target` — no bot in the audio path anymore.
...
A `Session` row reaches `status: completed` the moment the bot's `voice_session_completion` event lands — but the **post-call pipeline** (transcript persist → recording upload → extraction) keeps running for a few more seconds. The `processing_complete` boolean tells you when that pipeline is finished:

```
status                processing_complete    meaning
─────────────────────────────────────────────────────────────────────────────
active                false                  call still in progress
completed             false                  call ended; pipeline still running
...
async function fetchUntilReady(callId) {
  while (true) {
    ...
    await sleep(30_000); // 30s; pipeline usually finishes inside ~15s of completion
  }
}
```
