...
This event is sent when a partner-configured pre-answer policy is available for an inbound call.
...
| `call.disposition` | string or null | Final call outcome: `completed`, `voicemail`, `user_hangup_no_speech`, `no_answer`, `busy`, `canceled`, `blocked`, or `failed`. |
