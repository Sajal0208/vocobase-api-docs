...
<Warning>
  **Do not build a call record from `call.status.updated` alone.** This event is
  ...
  `hangup_cause` and `hangup_source` are frequently still `null`, and there is
  no end timestamp on this event at all — `occurred_at` marks the moment of the
  *state transition*, not the end of the call.
