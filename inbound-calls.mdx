...
### Which half of this page applies to you

Inbound is supported on **Twilio**, **Plivo**, **Vobiz**, and **Tata Smartflo**. MCube
inbound is not yet available. There are two routes, and you only need one:

| Your provider | Read |
|---|---|
| Twilio | [Set up inbound calling](#set-up-inbound-calling) — the console-driven flow below |
| Plivo, Vobiz, Tata Smartflo | [Use the V2 phone-numbers API](#use-the-v2-phone-numbers-api-plivo--vobiz--tata-smartflo) — sync DIDs and assign agents over the API |

Everything between the two — agent configuration, knowledge base, troubleshooting —
applies either way.
...
<Steps>
  ...
  <Step title="Assign an agent to a DID">
    `PUT /api/v2/phone-numbers/:id` with `agent_id` **and** `"inbound_enabled": true`. Inbound is explicit: assigning an agent on its own leaves the number's current inbound setting untouched, so a new DID stays outbound-only until you send the flag. `agent_id` is required whenever `inbound_enabled` is `true`. For outbound-only use with the same DID on another account, send `"inbound_enabled": false`.
