...
<Warning>
  Before testing Vobiz inbound parity in an environment that has just received this feature, make sure the backend migration `20260520030000_add_vobiz_application_id` has been applied. Without it, Vobiz assignment can fail because the `TelephonyConnection.vobizApplicationId` column is missing.
</Warning>
...
<Steps>
  ...
  <Step title="Assign an agent to a DID">
    `PUT /api/v2/phone-numbers/:id` with `agent_id`. For both Plivo and Vobiz, Vocobase calls the carrier's REST API to bind the DID to a Voice Application pointing at our inbound webhook — no portal paste needed.
    ...
    On Vobiz REST failure you'll get `502 VOBIZ_SYNC_FAILED` (Plivo: `502 PLIVO_SYNC_FAILED`). The local record is rolled back so retries are safe.
  </Step>

  <Step title="Reconcile with one call">
    If you edit something in the carrier's console directly (or rotate creds), `POST /api/v2/phone-numbers/resync` re-applies our Application binding to every assigned DID on that provider:
  ...
  <Step title="Receive the call">
    Call your DID. Carrier hits `/api/{plivo,vobiz}/{answer,inbound}` → Vocobase resolves the assigned agent → bot picks up. Same `session.completed` webhook payload as the Twilio flow above, just with `provider` in the metadata.
...
| `502 VOBIZ_SYNC_FAILED` | Vobiz REST credentials, Application creation, or DID attach failed | Confirm Vobiz credentials, DID ownership, and that the migration has been applied |
...
| Call connects but no webhook arrives | Partner webhook is not configured or endpoint rejects delivery | Configure `PUT /api/v2/config/webhook` and check your webhook server logs |
