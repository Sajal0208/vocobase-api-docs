...
<Note>
  If you create telephony connections through the V2 API, use the returned `connection_id` when syncing DIDs or starting outbound calls. A DID can have only one active owner globally, which keeps inbound routing deterministic.
</Note>
...
List connections when you need to choose a specific Plivo or Vobiz connection:

```bash
curl -sS "$BASE_URL/telephony/connections?provider=vobiz" \
  -H "Authorization: Bearer $API_KEY"
```
...
<Tip>
  See the per-provider setup guides for credential configuration — [Plivo Setup](/telephony/plivo-setup), [Vobiz Setup](/telephony/vobiz-setup). For multiple named connections, use [Telephony Connections](/telephony/connections).
</Tip>
...
| `409 PHONE_NUMBER_ALREADY_ACTIVE` | DID is already active on another connection | Disconnect the old owner or use a different DID |
