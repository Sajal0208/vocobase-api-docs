...
After the session ends, the platform fires a `session.completed` webhook containing the transcript, recording URL, credit usage, pre-call variables, and any post-call extraction. The `call` block is omitted for browser WebRTC sessions because there's no associated phone call.

Configure webhook endpoints with `POST /api/v2/config/webhooks`. Which endpoints receive the event depends on each endpoint's scope and event allowlist — an account-wide endpoint with no allowlist, the default, receives it for both telephony and WebRTC sessions. See [Webhook Setup](/webhooks/setup) for setup, [Scoping & Event Filtering](/webhooks/scoping) for routing, and [Webhook Payloads](/webhooks/payloads) for the full schema.
