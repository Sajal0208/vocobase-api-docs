...
Configure webhook endpoints with `POST /api/v2/config/webhooks`; enabled endpoints receive events for both telephony and WebRTC sessions. See [Webhook Setup](/webhooks/setup) for setup and [Webhook Payloads](/webhooks/payloads) for the full schema.
