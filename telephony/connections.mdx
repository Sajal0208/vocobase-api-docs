...
A phone number can have only one active owner globally. If the same DID is active on another connection, Vocobase rejects the new attachment so inbound routing stays deterministic.
