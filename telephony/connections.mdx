...
MCube connections take `jwt_token` and `exe_number` instead of account credentials — see [MCube Setup](/telephony/mcube-setup). Real phone-number exeNumbers are stored as canonical E.164; 4-15 digit short/internal identifiers remain digits. The exeNumber selects the MCube connection and is separate from the inbound DID list.
...
For Plivo, Vobiz, and MCube inbound DIDs, pass `connection_id` when syncing numbers for a specific connection. Vobiz and MCube accept an explicit `numbers` array. MCube accepts canonical E.164 or valid Indian national input, then stores and returns canonical E.164:
...
All three example values canonicalize to one row, `+918035214026`; the other values are reported as skipped. Explicit international values must include `+`, and invalid text, extensions, or vanity numbers are rejected. Use `GET /api/v2/phone-numbers?provider=MCUBE&connection_id=...` to retrieve the new row ID, then `PUT /api/v2/phone-numbers/{id}` with `agent_id` and `inbound_enabled: true` to make the DID inbound-ready. Normalization alone never creates, assigns, or enables a DID.
