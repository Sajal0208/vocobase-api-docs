...
`GET /api/v2/config` reports Exotel under `allowed_telephony_providers` and shows `telephony.exotel.configured: true` once your credentials are saved. See [Check provider access](/telephony/byop-setup#check-provider-access).
...
If your account has more than one active Exotel connection, also pass `connection_id` — omitting it returns `CONNECTION_AMBIGUOUS`. See [Bring Your Own Phone](/telephony/byop-setup#start-a-call) for the full request and response shape.
