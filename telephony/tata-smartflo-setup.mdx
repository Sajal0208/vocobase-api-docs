...
## Step 3: Confirm the account sees Tata Smartflo as configured

Fetch your account configuration. The response should now include Tata Smartflo in `allowed_telephony_providers`:
...
Start an outbound call with `"provider": "tata_smartflo"`. If your account has multiple active Tata Smartflo connections, include `connection_id`.
