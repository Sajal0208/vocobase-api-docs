...
## Step 3: Confirm the account sees Plivo as configured

Fetch your account configuration — the response should now include Plivo in `allowed_telephony_providers`:
...
Start an outbound call with `"provider": "plivo"`. If your account has multiple active Plivo connections, include `connection_id`.
...
Vocobase manages the carrier connection and media path for Plivo. Accounts only need to configure credentials and pass `"provider": "plivo"` when starting calls. Passing only `"provider": "plivo"` remains valid while there is exactly one active Plivo connection.
