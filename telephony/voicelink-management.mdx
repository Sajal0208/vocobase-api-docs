...
<Note>
  The signed token is a connection-scoped capability because VoiceLink does not attach a native request signature to the documented Generic WebSocket handshake. Vocobase verifies it before resolving a DID and scopes the lookup to that exact VoiceLink connection. Do not copy a generated bot URL into logs, tickets, or another connection.
</Note>
...
`steps[].step` is one of `resolve_did`, `verify_inbound_runtime`, `import_did`, `enable_inbound`, `client_mapping`, `ensure_bot`, `ensure_routing`, or `assign_agent`. `steps[].status` can be `done`, `already`, `skipped`, or `failed`. A `failed` step means the workflow stopped at that point and returned the latest readiness state so you can show the next operator action.
