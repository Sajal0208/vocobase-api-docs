...
## Configure VoiceLink in Vocobase
...
`GET /api/v2/config` reports VoiceLink under `allowed_telephony_providers` and shows `telephony.voicelink.configured: true` once your credentials are saved. See [Check provider access](/telephony/byop-setup#check-provider-access).
...
If your account has more than one active VoiceLink connection, also pass `connection_id` — omitting it returns `CONNECTION_AMBIGUOUS`. See [Bring Your Own Phone](/telephony/byop-setup#start-a-call) for the full request and response shape.
