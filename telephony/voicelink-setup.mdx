...
## Step 3: Confirm the account sees VoiceLink as configured

Fetch your account configuration — the response should now include VoiceLink in `allowed_telephony_providers`:
...
Start an outbound call with `"provider": "voicelink"`. If your account has multiple active VoiceLink connections, include `connection_id`.
...
VoiceLink uses a Generic WebSocket media model. Vocobase manages the media path; accounts only need to configure credentials and pass `"provider": "voicelink"` when starting calls. Passing only `"provider": "voicelink"` remains valid while there is exactly one active VoiceLink connection.
