...
- A Vocobase account with `"SIP"` present in `allowed_telephony_providers` (check `GET /api/v2/config`). If it is not listed, contact your Vocobase account manager to enable it.
...
## Gather your carrier credentials
...
## Add the SIP connection with the V2 API
...
To rotate credentials later, create a replacement connection and repoint your calls at the new `connection_id`. There is no `PUT /api/v2/config/telephony/sip` endpoint, and `PATCH /api/v2/telephony/connections/{connection_id}` only renames a connection — it does not rotate its credentials. Do **not** re-send the create request above: it adds a *second* SIP connection, which makes `/calls/start` fail with `CONNECTION_AMBIGUOUS`. See [Rename a connection](/telephony/connections#rename-a-connection).
...
If your account has more than one active SIP connection, also pass `connection_id` — omitting it returns `CONNECTION_AMBIGUOUS`. See [Bring Your Own Phone](/telephony/byop-setup#start-a-call) for the full request and response shape.
