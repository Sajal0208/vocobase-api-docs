---
title: "SIP (BYOP) Setup"
description: "Bring your own SIP carrier — configure inbound + outbound calling via LiveKit-mediated SIP trunking."
---

# SIP (BYOP) Setup

This guide walks you through connecting your own SIP carrier to Vocobase for inbound and outbound calling.

<Note>
  **SIP is a Bring-Your-Own-Provider (BYOP) transport.** You bring the carrier (Vobiz, Tata Tele, Knowlarity, or any ITSP that speaks standard SIP); Vocobase provisions LiveKit SIP trunks scoped to your connection and handles media termination. This provider coexists alongside the existing native Vobiz REST/WebSocket provider — pick whichever fits your carrier and tooling.
</Note>

## Prerequisites

- A Vocobase partner account with `"SIP"` present in `allowed_telephony_providers` (check `GET /api/v2/config`). If it is not listed, contact your Vocobase account manager to enable it.
- An account with any SIP carrier, with the following details ready:
  - **SIP host** — the carrier's SIP server hostname (e.g., `sip.your-carrier.com`)
  - **Outbound auth username** + **password** — the SIP credentials your carrier issues for INVITE authentication
  - **Source IPs** — the carrier's egress IPs from which inbound SIP INVITEs originate (used to allowlist inbound traffic)
  - At least one **DID** (phone number in E.164 format) provisioned on the carrier for inbound and/or outbound voice
- Access to the Vocobase dashboard at [app.vocobase.com](https://app.vocobase.com)

## Step 1: Gather your carrier credentials

<Steps>
  <Step title="Log in to your SIP carrier's portal">
    Sign in to your carrier's dashboard (Vobiz, Tata Tele, Knowlarity, or whichever ITSP you use).
  </Step>

  <Step title="Locate your SIP trunk settings">
    Find the SIP trunk / SIP gateway section. You are looking for:

    - **SIP host** — typically a hostname like `sip.carrier.com` or `pbx.carrier.com`
    - **Auth username** and **auth password** — the credentials Vocobase will present when placing outbound INVITEs through your carrier
    - **Source IPs** — the IPs your carrier uses to send inbound INVITEs to subscribers. Most carriers publish these in their docs.

    <Warning>
      Your SIP auth password grants outbound calling on your trunk. Treat it like any production secret.
    </Warning>
  </Step>

  <Step title="Confirm your DID(s)">
    From the carrier portal, note each phone number you plan to use, in E.164 format (e.g., `+918011223344`). These are the numbers Vocobase will use as caller IDs on outbound calls and bind to inbound routing on inbound calls.
  </Step>
</Steps>

## Step 2: Add the SIP connection in Vocobase

<Steps>
  <Step title="Open the dashboard">
    Go to [app.vocobase.com](https://app.vocobase.com) and sign in.
  </Step>

  <Step title="Add a SIP connection">
    Navigate to **Settings → Telephony Settings** and click **Add Connection**. Pick **SIP** from the provider list.

    Fill in:

    - **Connection name** — a label for this trunk (e.g., `Vobiz SIP — Production`, `Tata Tele India`)
    - **SIP host** — from Step 1
    - **Auth username** + **password** — from Step 1
    - **Source IPs** — comma-separated, from Step 1
    - **DIDs** — the E.164 phone numbers you provisioned
  </Step>

  <Step title="Save and copy the LiveKit SIP URI">
    On **Save**, Vocobase provisions per-connection inbound and outbound trunks on LiveKit Cloud and displays the **LiveKit SIP URI** (something like `sip:<projectId>.sip.livekit.cloud`).

    Paste that URI into your carrier's DID-origination / outbound routing setting so that calls to your DIDs are forwarded to LiveKit. The exact field name varies by carrier (often called *origination URI*, *SIP termination URL*, or *destination SIP URI*).
  </Step>
</Steps>

## Step 3: Make an outbound test call

Start an outbound call with `"provider": "sip"`:

<CodeGroup>
```bash cURL
curl -X POST https://api.vocobase.com/api/v2/calls/start \
  -H "Authorization: Bearer rg_live_abc123def456ghi789jkl012" \
  -H "Content-Type: application/json" \
  -d '{
    "agent_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
    "to_number": "+919876543210",
    "provider": "sip"
  }'
```

```javascript JavaScript
const response = await fetch(
  "https://api.vocobase.com/api/v2/calls/start",
  {
    method: "POST",
    headers: {
      "Authorization": "Bearer rg_live_abc123def456ghi789jkl012",
      "Content-Type": "application/json"
    },
    body: JSON.stringify({
      agent_id: "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
      to_number: "+919876543210",
      provider: "sip"
    })
  }
);
const result = await response.json();
console.log(result);
```

```python Python
import requests

response = requests.post(
    "https://api.vocobase.com/api/v2/calls/start",
    headers={
        "Authorization": "Bearer rg_live_abc123def456ghi789jkl012",
        "Content-Type": "application/json"
    },
    json={
        "agent_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
        "to_number": "+919876543210",
        "provider": "sip"
    }
)
print(response.json())
```
</CodeGroup>

A successful response looks like:

```json
{
  "success": true,
  "data": {
    "call_id": "c1234567-abcd-1234-abcd-123456789012",
    "session_id": "s1234567-abcd-1234-abcd-123456789012",
    "status": "pending",
    "provider": "sip",
    "from_number": "+918011223344",
    "to_number": "+919876543210",
    "agent_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
  }
}
```

## How it works

- **Outbound** — Vocobase dials your agent into a LiveKit room and instructs LiveKit's SIP service to place a SIP INVITE through the per-connection outbound trunk to your carrier. Your carrier delivers the call to the PSTN.
- **Inbound** — When a caller dials one of your DIDs, your carrier forwards the SIP INVITE to the LiveKit SIP URI you configured. LiveKit matches it to your per-connection inbound trunk, Vocobase routes it to the correct agent based on the dialed DID, and the bot joins the LiveKit room.
- **Transfer** — Cold transfers ride on the same trunk: the bot issues a SIP REFER to the carrier with the target number, the carrier bridges the caller to the transfer destination, and the bot leaves the room.

## Next steps

<CardGroup cols={2}>
  <Card title="Vobiz Setup" icon="phone" href="/telephony/vobiz-setup">
    Native Vobiz REST/WebSocket integration (no SIP trunking required).
  </Card>
  <Card title="Quick Start" icon="rocket" href="/quickstart">
    Create an agent and make your first call.
  </Card>
</CardGroup>
