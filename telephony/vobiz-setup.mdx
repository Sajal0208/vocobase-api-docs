...
## Step 3: Confirm the account sees Vobiz as configured

Fetch your account configuration — the response should now include Vobiz in `allowed_telephony_providers`:
...
Start an outbound call with `"provider": "vobiz"`. If your account has multiple active Vobiz connections, include `connection_id`.
...
Vocobase manages the carrier connection and media path for Vobiz. Accounts only need to configure credentials and pass `"provider": "vobiz"` when starting calls. Passing only `"provider": "vobiz"` remains valid while there is exactly one active Vobiz connection.
