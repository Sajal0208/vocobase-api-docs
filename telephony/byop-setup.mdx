...
Calls are billed at the resolved per-tier rate (VA 1 default ₹3.60/min, VA 1 Pro default ₹4.60/min) in **15-second buckets**. ₹ is deducted after the call ends.

Calls that fail to connect (`NO_ANSWER`, `BUSY`, `FAILED` before the customer answers) do not consume ₹.

Your telephony provider (Twilio/Exotel) bills you separately for phone usage.

See [Billing (₹)](/credits) for full details, including tier resolution and per-account/project/agent overrides.
...
| "Insufficient balance" | Top up your Vocobase ₹ balance. This is separate from your Twilio/Exotel balance. |
...
<CardGroup cols={2}>
  ...
  <Card title="Billing (₹)" icon="coins" href="/credits">
    Understand how calls consume ₹ — tiers, 15s buckets, and overrides.
  </Card>
