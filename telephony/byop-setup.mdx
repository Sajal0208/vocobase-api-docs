...
```json
{
  "success": true,
  "data": {
    "allowed_telephony_providers": ["twilio", "plivo", "vobiz", "tata_smartflo"],
    "telephony": {
      ...
      "tata_smartflo": {
        ...
        "from_number": null,
        "agent_number": null,
        "click_to_call_api_key_set": false
      }
    }
  }
}
...
If the provider you need is missing from `allowed_telephony_providers`, contact Vocobase support to enable it for your account.
...
After the provider is enabled and configured, start a call with `POST /api/v2/calls/start` and pass the provider value. If the account has multiple active connections for that provider, also pass `connection_id`.
...
For inbound calling with Plivo or Vobiz, use the V2 phone-numbers API to sync DIDs and assign agents. MCube DIDs can be registered the same way — its inventory is added manually, so it takes an explicit E.164 `numbers` array — but MCube **inbound is not currently available**; registering the DID does not make it ring. See [Inbound Calls](/inbound-calls).
...
| Symptom | Action |
|---------|--------|
| Provider is not allowed | Ask Vocobase support to enable the provider for your account. |
