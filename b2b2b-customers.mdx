...
If you resell Vocobase to downstream customers of your own, every customer needs to authorize tools (Google Sheets, Slack, Shopify, Cal.com, Zendesk, LeadSquared, etc.) **with their own credentials, isolated from each other**. The Vocobase B2B2B API gives each of your customers their own identity (`Customer`) and their own connections, while you remain the single billing and integration boundary with us.

<Info>
  **Mental model.** Vocobase ↔ you ↔ your customers. You authenticate to Vocobase with your `rg_live_…` API key. Your customers never log in to Vocobase. They authorize tools through one-time hosted URLs that you mint and hand off to them.
</Info>
...
If you are a single team using your own tool credentials, **you do not need this surface** — connect tools directly under your account from the dashboard at `/dashboard/settings/integrations`.
...
| Term | What it is |
|------|-----------|
| **Account** | You. Holds the `rg_live_…` API key, billing relationship, agent quota. |
| **Customer** | One of your downstream customers. Has a stable `external_id` you choose. |
...
| **Connection** | A specific authorization of a connector for a specific owner. Owners are mutually exclusive — either you (the account) OR a single Customer. |
| **Connect link** | A one-time URL you mint and hand to a customer. They open it, complete OAuth (or paste an API key), and tokens land against the Customer. |

---

## Discover available connectors

Before you can offer connectors to your customers, list the catalog your account has access to. Admin controls visibility per account — if a connector you expect is missing, ask the Vocobase team to grant it.
...
<Note>
  Only `ACTIVE` connectors are returned. The dashboard may show `COMING_SOON` previews; the API filters those out because they have no working programmatic OAuth path.
</Note>
...
Customers are identified by a stable `external_id` of your choice — your own primary key for that downstream account. Re-creating with the same `(account, external_id)` pair is **idempotent**, so a sync that runs on every deploy is safe.
...
[Your backend]                          [Vocobase]                       [Customer's browser]
     ...
     │                                        │                                  │
     │                                        │ Customer authorizes              │
     │                                        │ Tokens land against the Customer
     │                                        │                                  │
...
curl -X POST "https://api.vocobase.com/api/v2/customers/pc_01h.../connect-link" \
  ...
  -d '{
    ...
    "redirect_uri": "https://app.example.com/integrations/return",
...
- Host must EXACT-match an entry in your `allowed_redirect_hosts`. If unset, falls back to the host of your `webhook_url`. Ask Vocobase to register additional hosts on your account.
...
| 404 | `NOT_FOUND` | Customer or connector doesn't exist or doesn't belong to your account. |
...
| Redirect host allowlist | configured per account — ask Vocobase to add hosts |
...
<Note>
  Two wire names keep a legacy `partnerCustomer` prefix — the owner discriminator
  `"kind": "partnerCustomer"` and the `partnerCustomerId` field. They are
  unchanged and still correct; only the wording in these docs moved to
  "customer". Do not rename them in your integration.
</Note>

- **Per-customer billing.** ₹ is billed to your account, not to individual customers. (Track per-customer usage from session metadata or session listing.)
...
- **Per-customer webhook routing.** Your single `webhook_url` receives events for all customers, and the payload carries no customer identifier. Attribute them the same way as billing — via session metadata or the session listing.
